...
class CommandLineJavaCompilerArgumentsGeneratorTest extends Specification {
    ...
    def "inlines arguments if they are short enough"() {
        ...
        then:
        Lists.newArrayList(args) == ["-J-Xmx256m", "-g", "-sourcepath", defaultEmptySourcePathRefFolder(), "-classpath", spec.classpath.asPath, *spec.source*.path.sort(), USE_UNSHARED_COMPILER_TABLE_OPTION]
    }
    ...
    def "creates arguments file if arguments get too long"() {
        ...
        argsFile.readLines() == ["-g", "-sourcepath", quote(defaultEmptySourcePathRefFolder()), "-classpath", quote("$spec.classpath.asPath"), *(spec.source*.path.sort().collect { quote(it) }), USE_UNSHARED_COMPILER_TABLE_OPTION]
    }
